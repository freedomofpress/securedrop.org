...
class OrderedFeatures(Orderable):
    page = ParentalKey("marketing.MarketingIndexPage", related_name="features")
...
class FeaturePage(MetadataPageMixin, Page):
    ...
    def next(self):
        ordered_feature = OrderedFeatures.objects.filter(
            page=self.get_parent(), sort_order__gt=self.sort_order()
        ).first()
        if ordered_feature:
    ...
    def previous(self):
        ordered_feature = OrderedFeatures.objects.filter(
            page=self.get_parent(), sort_order__lt=self.sort_order()
        ).last()
        if ordered_feature:
            return ordered_feature.feature
        return None
