...
class FormField(AbstractFormField):
    ...
    image = models.ForeignKey(
        "common.CustomImage", null=True, blank=True, on_delete=models.SET_NULL
    )
    image_caption = RichTextField(blank=True, null=True)
    image_link_text = models.CharField(
        blank=True, null=True, default="Show Image", max_length=50
    )
...
class FormPage(MetadataPageMixin, HoneypotFormMixin, HoneypotFormSubmissionMixin):
    ...
    warning = RichTextField(
        blank=True,
        help_text="A warning for sources not to submit documents via this form.",
    )
